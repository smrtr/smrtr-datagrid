 ...
 * DataGrid
 *
 * 2D-array wrapper with methods for import/export.
 *
 * @author Joe Green <joe.green@smrtr.co.uk>
 * @package Smrtr
 * @link https://smrtr.github.io/smrtr-datagrid/
 */
class DataGrid implements \Serializable
{
    /**
     * @var int On loadArray(), associate row labels by using array keys
     */
    const ASSOC_ROW_KEYS = 1;

    /**
     * @var int On loadArray(), associate row labels by using the first column
     */
    const ASSOC_ROW_FIRST = 0;

    /**
     * @var int On loadArray(), associate column labels by using array keys
     */
    const ASSOC_COLUMN_KEYS = 0;

    /**
     * @var int On loadArray(), associate column labels by using the first row
     */
    const ASSOC_COLUMN_FIRST = 1;

    /**
     * @var int Maximum length of operator string
     */
    const MAX_OPERATOR_LENGTH = 2;

    /**
     * @var int Maximum length of value string
     */
    const MAX_VALUE_LENGTH = 1000;

    /**
     * @var int Constant for left associativity of operators
     */
    const LEFT_ASSOCIATIVE = 0;

    /**
     * @var int Constant for right associativity of operators
     */
    ...
    /**
     * @var array A map of search operators to matching-functions
     */
    private static $searchSelectors;

    /**
     * @var array An array cache of search selector-operator characters
     */
    private static $searchSelectorChars;

    /**
     * @var array Supported operators ["operator" => [precedence, associativity]]
     */
    private static $setOperators = array(
        "+" => array(   10,     self::LEFT_ASSOCIATIVE    ),     // intersection
        "-" => array(   10,     self::LEFT_ASSOCIATIVE    ),     // difference
        "," => array(   0,      self::LEFT_ASSOCIATIVE    )      // union
    );
    ...
    /**
     * @var int A count of columns
     */
    protected $columns;
    
    /**
     * @var int A count of rows
     */
    protected $rows;
    
    /**
     * @var array A map from column keys to column labels or null
     */
    protected $columnKeys = array();
    
    /**
     * @var array A map from row keys to row labels or null
     */
    protected $rowKeys = array();
    
    /**
     * @var array A 2-Dimensional array of data
     */
    protected $data = array();

    /**
     * @var bool Restrict data values to scalar types? Turned on by default.
     */
     ...
     *
     * @param string $token
     * @param int    $type  One of the (LEFT|RIGHT)_ASSOCIATIVE constants
     *
     * @return bool
     *
    ...
    private static function isAssociative($token, $type)
    {
        if (!self::isSetOperator($token)) {
            throw new DataGridException("Invalid token: $token");
        }
        if ($type === self::$setOperators[$token][1]) {
            return true;
        }
        return false;
    }
    ...
    {
        if (is_null(self::$searchSelectors)) {
            self::$searchSelectors = array(
                '='  => function ($val1, $val2) {
                    return ($val1 == $val2);
                },
                '!=' => function ($val1, $val2) {
    ...
    {
        if (is_null(self::$searchSelectorChars)) {
            self::$searchSelectorChars = array();
            $operators = array_keys(self::selectors());
            foreach ($operators as $operator) {
                for ($n=0; $n < strlen($operator); $n++) {
                    if (! in_array($operator[$n], self::$searchSelectorChars)) {
                        self::$searchSelectorChars[] = $operator[$n];
                    }
                }
            }
        }
        return self::$searchSelectorChars;
    }
        ...
        foreach ($tokens as $token) {
            if (self::isSetOperator($token)) {
                while (count($stack) && self::isSetOperator($stack[0])) {
                    if (
                        (
                            self::isAssociative($token, self::LEFT_ASSOCIATIVE)
                            && self::compareSetOperatorPrecedence($token, $stack[0]) <= 0
                        )
                        || (
                            self::isAssociative($token, self::RIGHT_ASSOCIATIVE)
                            && self::compareSetOperatorPrecedence($token, $stack[0]) < 0
                        )
                    ) {
     ...
     *
     * @param string   $str                Passed by reference
     * @param string[] $selectorCharacters
     *
     * @return string
     */
    private function extractSearchOperation(&$str, array $selectorCharacters)
    {
        $n = 0;
        $operator = '';
        while (isset($str[$n]) && in_array($str[$n], $selectorCharacters) && $n < self::MAX_OPERATOR_LENGTH) {
            $operator.= $str[$n];
            $n++;
        }
        if ($operator) {
    ...
    private function extractSearchValue(&$str)
    {
        ...
        do {
            ...
            $c = $str[$n];
            if ($openingQuote) {   // we are in a quoted value string
                if ($c == $openingQuote) {
                    if ($lastChar != '\\') {   // same quote as opening quote, and not escaped = closing quote
                        $n++;
                        break;
                    } else {   // intentionally escaped quote (remove the escape char)
                        $value = rtrim($value, '\\');
                    }
                }
            } else {   // we are in an unquoted value string
                if ($c == '|') {
    ...
    private function evaluateSearchVector($v, $key, $label, $rowOrColumn, $selector)
    {
        ...
        if ('row' == $rowOrColumn) {
            $rowOrColumnInverse = 'column';
        } elseif ('column' == $rowOrColumn) {
            $rowOrColumnInverse = 'row';
        } else {
        ...
        foreach ($fields as $field) {
            foreach ($values as $value) {
                if ('/' == $field) {
                    ...
                    $val1 = $label;
                } elseif (preg_match('#/(\d+)#', $field, $matches)) {
                    $val1 = $v[$this->getKey($rowOrColumnInverse, (int)$matches[1])];
                } else {
                    $val1 = $v[$this->getKey($rowOrColumnInverse, $field)];
                }
     ...
     *
     * Returns results as a new DataGrid instance.
     *
     * @link https://smrtr.github.io/smrtr-datagrid/searching.html
     *
    ...
    {
        if (!is_string($str)) {
            throw new DataGridException("String expected");
        }
        if (!strlen($str)) {
            return $this;
        }
     ...
     *
     * @param string      $rowOrColumn
     * @param string|null $label         Null by default
     * @param bool        $increaseCount False by default
     *
     * @return static
     *
     * @throws DataGridException
     ...
     *
     * @param string $rowOrColumn
     * @param array  $labels        Null by default
     * @param bool   $increaseCount False by default
     *
     * @return void
     *
    ...
    /**
     * Get or set row labels
     *
     * @api
     *
     * @param array|bool $labels False by default
     *
     * @return array|static
     *
     * @throws DataGridException
